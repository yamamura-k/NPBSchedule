...
class Generate(NPB):
    def __init__(self) -> None:
        super().__init__()
    def GenerateFeasibleSolution(self, game_type, league='ps', timeLimit=None, solverName=None, threads=1, option=[], initialPosition=None, initialSolution=False):
        """
        This is a function to solve 0-1 integer problem to get a schedule.
        ...
        # デフォルトは cbc solver
        if solverName == 0:
            solver = pulp.PULP_CBC_CMD(msg=0, threads=threads,options=option, maxSeconds=timeLimit, warmStart=True)
        elif solverName == 1:
            solver = pulp.CPLEX_CMD(msg=0, timeLimit=timeLimit,options=option, threads=threads, warmStart=True)
